...
# Carga persistente de tabla maestra
...
# ============================================================
# 🖨️ EXPORTACIÓN DE INFORMES PDF / WORD (con formato institucional)
# ============================================================

with st.expander("🖨️ Generar informe PDF / Word", expanded=False):
    st.header("🖨️ Generar Informe con Gráficos y Mapa")

    if not st.session_state["tabla_maestra"].empty:
        df_export = df_localidad.copy() if not df_localidad.empty else st.session_state["tabla_maestra"].copy()

        col_exp1, col_exp2 = st.columns(2)
        formato = col_exp1.radio("Formato de exportación", ["Word (.docx)", "PDF (.pdf)"], horizontal=True)
        incluir_mapa = col_exp2.checkbox("Incluir mapa semaforizado", value=True)
        ...
        if st.button("📄 Generar Informe"):
            ...
            if formato == "Word (.docx)":
                ...
                header_para.alignment = 1  # centro

                # Fondo azul en encabezado
                shading_elm = parse_xml(r'<w:shd {} w:fill="004AAD"/>'.format(nsdecls('w')))
                header_para._element.get_or_add_pPr().append(shading_elm)

                run = header_para.add_run()
                if os.path.exists("logo_enacom.png"):
                    run.add_picture("logo_enacom.png", width=Inches(2.3))
                header_para.add_run("\n")
                header_para.add_run("ENTE NACIONAL DE COMUNICACIONES (ENACOM)").bold = True
                ...
                if not df_export.empty and "Resultado" in df_export.columns:
                    doc.add_paragraph(f"Resultado máximo (V/m): {df_export['Resultado'].max():.2f}")

                doc.add_paragraph(" ")

                # --- Agregar gráfico principal ---
                img_bytes = BytesIO()
                pio.write_image(fig_bar, img_bytes, format="png")
                img_bytes.seek(0)
                doc.add_picture(img_bytes, width=Inches(5.5))
                doc.add_paragraph("Gráfico de Localidades por Provincia y CCTE.")

                # --- Mapa semaforizado ---
                if incluir_mapa and os.path.exists("mapa de calor.png"):
                    doc.add_picture("mapa de calor.png", width=Inches(5.5))
                    doc.add_paragraph("Mapa semaforizado de resultados.")

                # --- Pie institucional ---
                section.footer.paragraphs[0].text = "Base de datos de mediciones RNI - ENACOM | Generado automáticamente"
            ...
            # ============================================================
                ...
                # --- Encabezado visual azul con logo ---
                if os.path.exists("logo_enacom.png"):
                    story.append(RLImage("logo_enacom.png", width=200, height=60))
                story.append(Paragraph("<b>ENTE NACIONAL DE COMUNICACIONES (ENACOM)</b>", style_title))
                story.append(Paragraph("Informe de Mediciones RNI", style_sub))
                story.append(Spacer(1, 20))

                # --- Datos generales ---
                story.append(Paragraph(f"<b>Localidad:</b> {localidad_nombre}", style_normal))
                ...
                if not df_export.empty and "Resultado" in df_export.columns:
                    story.append(Paragraph(f"<b>Resultado máximo (V/m):</b> {df_export['Resultado'].max():.2f}", style_normal))
                story.append(Spacer(1, 20))
                ...
                story.append(Paragraph("Gráfico de Localidades por Provincia y CCTE", styles["Italic"]))

                # --- Mapa semaforizado ---
                if incluir_mapa and os.path.exists("mapa de calor.png"):
                    story.append(Spacer(1, 20))
                    story.append(RLImage("mapa de calor.png", width=400, height=250))
                    story.append(Paragraph("Mapa semaforizado de resultados", styles["Italic"]))

                story.append(Spacer(1, 30))
                story.append(Paragraph("<font size=9 color='#004AAD'>Base de datos de mediciones RNI - ENACOM</font>", style_sub))
